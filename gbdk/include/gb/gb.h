...
#define SWITCH_ROM_MBC1(b) \
  *(const unsigned char *)0x2000 = (b)

#define SWITCH_RAM_MBC1(b) \
  *(const unsigned char *)0x4000 = (b)

#define ENABLE_RAM_MBC1 \
  *(const unsigned char *)0x0000 = 0x0A

#define DISABLE_RAM_MBC1 \
  *(const unsigned char *)0x0000 = 0x00
...
#define SWITCH_ROM_MBC5(b) \
  *(const unsigned char *)0x3000 = (UINT16)(b)>>8; \
  *(const unsigned char *)0x2000 = (UINT8)(b)
...
/** Sets the tiles in the background tile table.
    ...
    @param w		Range 0 - 31
    @param h		Range 0 - 31
    @param data		Pointer to an const unsigned char. Usually the 
...
void
set_data(const unsigned char *vram_addr,
	 const unsigned char *data,
	 UINT16 len);

void
get_data(const unsigned char *data,
	 const unsigned char *vram_addr,
	 UINT16 len);
